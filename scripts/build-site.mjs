...
const files=[];
async function collect(dir=''){for(const entry of await readdir(join('public',dir),{withFileTypes:true})){const path=join(dir,entry.name);if(entry.isDirectory())await collect(path);else files.push(path);}}
